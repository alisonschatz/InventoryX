...
import { Eye, EyeOff, Mail, Lock, User, Loader2, UserCheck } from 'lucide-react'
...
export default function LoginPage() {
  ...
  const handleGuestLogin = async () => {
    ...
    try {
      ...
      // Criar um usuário convidado temporário no localStorage
      const guestUser = {
        uid: 'guest-' + Date.now(),
      ...
      localStorage.setItem('inventoryx-guest-user', JSON.stringify(guestUser))
      localStorage.setItem('inventoryx-guest-mode', 'true')
      
      // Recarregar a página para que o AuthContext detecte o modo convidado
      window.location.reload()
      
    } catch (err: any) {
      console.error('❌ Erro no modo convidado:', err)
        ...
        <div className="bg-theme-panel rounded-2xl shadow-theme-medium p-8 border border-theme-soft">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              {/* Botão Modo Convidado */}
              <button
                type="button"
                onClick={handleGuestLogin}
                disabled={isLoading || isGuestLoading}
                className="w-full bg-theme-hover hover:bg-theme-soft border border-theme-soft hover:border-purple-300 dark:hover:border-purple-600 text-theme-primary font-medium py-3 px-4 rounded-xl transition-all shadow-sm flex items-center justify-center gap-3 disabled:opacity-50"
              >
                {isGuestLoading ? (
                  <>
                    <Loader2 className="w-5 h-5 animate-spin" />
          ...
          {/* Informação sobre modo convidado */}
          <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
            <p className="text-xs text-blue-700 dark:text-blue-400 text-center">
              💡 <strong>Modo Convidado:</strong> Explore o InventoryX sem cadastro! Seus dados serão salvos localmente.
            </p>
          </div>
        </div>
      </div>
